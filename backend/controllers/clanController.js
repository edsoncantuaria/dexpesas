...
const serializeBigInts = (data) => {
    ...
    if (data !== null && typeof data === 'object') {
        ...
        for (const key in data) {
            ...
            } else if (data[key] !== null && typeof data[key] === 'object' && !Buffer.isBuffer(data[key])) {
...
class ClanController {
    ...
    async createClan(req, res, next) {
        ...
        try {
            ...
            if (user.clanMembership) {
                return res.status(400).json({ message: 'Você já faz parte de uma família. Saia da família atual para criar uma nova.' });
            }
            ...
            await AuditService.log({ userId, action: 'CREATE_CLAN', entity: 'CLAN', entityId: newClan.id, details: { after: serializeBigInts(newClan) }, ipAddress: req.ip });
            res.status(201).json(serializeBigInts(newClan));
        } catch (error) {
            if (error.code === 'P2002') {
                return res.status(409).json({ message: `Uma família com o nome "${name}" já existe.` });
            }
            next(error);
        }
    }
    ...
    async updateClan(req, res, next) {
        ...
        try {
            ...
            if (!member || member.role !== 'LEADER') {
                return res.status(403).json({ message: 'Apenas o líder pode editar a família.' });
            }
    ...
    async deleteClan(req, res, next) {
        ...
        try {
            const member = await prisma.clanMember.findFirst({ where: { userId, clanId } });
            if (!member || member.role !== 'LEADER') {
                return res.status(403).json({ message: 'Apenas o líder pode dissolver a família.' });
            }
        ...
        try {
            ...
            const { auditLog } = await prisma.$transaction(async (tx) => {
                ...
                const auditLog = await AuditService.log({
                    ...
                    details: { amount, expenseDescription: description, memberName: member.user.name, categoryId }
                });
                 return { auditLog };
            });
            
            res.status(200).json({ message: 'Despesa registrada com sucesso.', auditLogId: auditLog.id });
        } catch (error) {
    ...
    async reverseClanExpense(req, res, next) {
        ...
        try {
            const member = await prisma.clanMember.findFirst({ where: { userId, clanId } });
            if (!member || !['LEADER', 'ADMIN'].includes(member.role)) {
                return res.status(403).json({ message: 'Apenas líderes ou admins podem reverter despesas.' });
            }

            const logToReverse = await prisma.auditLog.findUnique({ where: { id: auditLogId } });
            if (!logToReverse || logToReverse.action !== 'CLAN_EXPENSE' || logToReverse.entityId !== clanId) {
                 return res.status(404).json({ message: 'Registro de despesa inválido ou não encontrado.' });
            }
            if ((logToReverse.details as any)?.reversed) {
                 return res.status(400).json({ message: 'Esta despesa já foi revertida.' });
            }

            const { amount, expenseDescription } = logToReverse.details as any;
            ...
            await prisma.$transaction(async (tx) => {
                ...
                await tx.auditLog.update({
                    where: { id: auditLogId },
                    data: { details: { ...logToReverse.details, reversed: true, reversedBy: userId } }
                });
                 await AuditService.log({
                    userId, action: 'REVERSE_CLAN_EXPENSE', entity: 'CLAN', entityId: clanId, ipAddress: req.ip,
                    details: { amount, reversedLogId: auditLogId, reversedDescription: expenseDescription }
                });
            });
        ...
        try {
            ...
            await prisma.$transaction(async (tx) => {
                ...
                if (Number(clan.balance) < totalAmount) {
                    throw { statusCode: 400, message: 'Saldo da família insuficiente para esta despesa.' };
                }
                ...
                if (membersCount === 0) throw { statusCode: 400, message: 'Não há membros na família para ratear.' };
         ...
         try {
            await prisma.$transaction(async (tx) => {
                ...
                if (goal.status === 'COMPLETED' || Number(goal.currentAmount) >= Number(goal.targetAmount)) {
                    throw { statusCode: 400, message: 'Esta meta já foi alcançada. Não é possível fazer novas contribuições.' };
                }

                await tx.transaction.create({
                    data: {
                        userId, accountId: fromAccountId, descricao: `Contribuição para meta da Família`, valor: amount,
        ...
        try {
            const logs = await prisma.auditLog.findMany({
                where: {
                    entity: 'CLAN',
                    entityId: clanId,
                    action: { in: ['CLAN_CONTRIBUTION', 'CLAN_EXPENSE', 'REVERSE_CLAN_EXPENSE'] }
                },
    ...
    async inviteMember(req, res, next) {
        ...
        try {
            const inviter = await prisma.clanMember.findFirst({ where: { userId: inviterId, clanId } });
            if (!inviter || !['LEADER', 'ADMIN'].includes(inviter.role)) {
                return res.status(403).json({ message: 'Você não tem permissão para convidar para esta família.' });
            }

            const invitedUser = await prisma.user.findUnique({ where: { id: invitedUserId }, include: { clanMembership: true } });
            if (!invitedUser) return res.status(404).json({ message: 'O jogador convidado não foi encontrado.' });
            if (invitedUser.clanMembership) return res.status(400).json({ message: 'Este jogador já faz parte de uma família.' });
            ...
            if (error.code === 'P2002') return res.status(409).json({ message: 'Um convite para este jogador já está pendente.' });
            next(error);
        }
    }
    ...
    async getPendingInvites(req, res, next) {
        try {
            const invites = await prisma.clanInvite.findMany({
                where: { invitedUserId: req.user.id, status: 'PENDING', expiresAt: { gte: new Date() } },
                include: { clan: true },
                orderBy: { createdAt: 'desc' }
            });
            res.json(invites);
    ...
    async acceptInvite(req, res, next) {
        ...
        try {
            await prisma.$transaction(async (tx) => {
                ...
                await tx.clanMember.create({ data: { userId, clanId: invite.clanId, role: 'MEMBER' } });
                await tx.clanInvite.update({ where: { id: inviteId }, data: { status: 'ACCEPTED' } });
            });
    ...
    async leaveClan(req, res, next) {
        ...
        const userId = req.user.id;
        try {
            const member = await prisma.clanMember.findUnique({ where: { userId_clanId: { userId, clanId } }});
            if (!member) return res.status(404).json({ message: 'Você não é membro desta família.' });
            if (member.role === 'LEADER') {
                return res.status(400).json({ message: 'O líder não pode sair da família. Transfira a liderança primeiro.' });
            }
        ...
        try {
            await prisma.$transaction(async (tx) => {
                ...
                if (!currentLeader || currentLeader.role !== 'LEADER') {
                    throw { statusCode: 403, message: 'Apenas o líder pode transferir a liderança.' };
                }
                ...
                if (!newLeader) {
                    throw { statusCode: 404, message: 'O membro selecionado para ser o novo líder não foi encontrado.' };
                }
    ...
    async exportClanReport(req, res, next) {
        ...
        try {
            const member = await prisma.clanMember.findFirst({ where: { userId, clanId }});
            if (!member || !['LEADER', 'ADMIN'].includes(member.role)) {
                return res.status(403).json({ message: 'Apenas líderes ou admins podem exportar relatórios.' });
            }
            ...
            const dataToExport = logs.map(log => {
                ...
                return {
                    ...
                    Descricao: details.expenseDescription || `Contribuição de ${details.memberName}`
                }
            });
            
            const csv = stringifyCsv(dataToExport, { header: true });
            res.setHeader('Content-Type', 'text/csv');
            res.setHeader('Content-Disposition', `attachment; filename=extrato_cla_${clanId}.csv`);
