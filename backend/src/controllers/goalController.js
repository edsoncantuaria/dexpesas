...
class GoalController {
    ...
    /**
     * Deleta uma meta.
     * Melhoria: Agora também exclui as transações de débito associadas às contribuições.
     */
    ...
    async addContribution(req, res, next) {
        ...
        try {
            const result = await prisma.$transaction(async (tx) => {
                ...
                const currentBalance = Number(sourceAccount.saldoInicial) + sourceAccount.transactions.reduce((acc, t) => acc + (t.tipo === 'receita' ? Number(t.valor) : -Number(t.valor)), 0);
                ...
                await GamificationService.triggerXpEvent(tx, userId, 'GOAL_CONTRIBUTION', { amount: parsedAmount });
                // Causa dano ao chefe com base no valor da contribuição
        ...
        try {
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (updatedGoal.status === 'COMPLETED') {
                    await GamificationService.triggerXpEvent(tx, userId, 'GOAL_COMPLETED');
