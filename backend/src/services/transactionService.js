...
class TransactionService {
    ...
    static async handleBillPayment(userId, cardId, accountId, paymentAmount, paymentDate, prismaTx) {
        ...
        return executor(async (transactionTx) => {
            ...
            const card = await transactionTx.card.findUnique({ where: { id: cardId } });
            if (!card) {
            ...
            const invoiceTransactions = await transactionTx.transaction.findMany({
            ...
            const existingPayment = await transactionTx.transaction.findFirst({
                where: {
                    accountId: accountId,
            ...
            const expenseTransaction = await transactionTx.transaction.create({
                data: {
                    userId, accountId,
                    descricao: `Pagamento Fatura ${card.nome}`,
            ...
            const incomeTransaction = await transactionTx.transaction.create({
                data: {
                    userId, cardId,
            ...
            if (remainingBalance > 0 && card.jurosRotativo && card.jurosRotativo > 0) {
                ...
                await transactionTx.transaction.create({
                    data: {
                        userId, cardId,
