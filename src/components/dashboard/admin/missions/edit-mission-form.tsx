            ...
            <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-4">
                ...
                <div className="space-y-2">
                    <div className="flex justify-between items-end">
                        <FormLabel>Gatilho (JSON)</FormLabel>
                        <Button
                            type="button"
                            variant="outline"
                            size="sm"
                            ...
                            onClick={() => {
                                form.setValue('triggerSpec', JSON.stringify({
                                    type: "TRANSACTION_CREATED",
                                    count: 30,
                                    period: "monthly"
                                }, null, 2));
                                form.setValue('isRepeatable', true);
                                form.setValue('title', 'Maratona Mensal: ');
                            }}
                        >
                            Usar Preset Mensal
                        </Button>
                    </div>
                    <FormField control={form.control} name="triggerSpec" render={({ field }) => (<FormItem><FormControl><Textarea placeholder='{"type": "TRANSACTION_CREATED", "count": 5}' {...field} rows={4} className="font-mono text-xs" /></FormControl><FormDescription>Ex: Completar ao criar 5 transações.</FormDescription><FormMessage /></FormItem>)} />
                </div>
