// src/components/dashboard/clans/goal-contribution-dialog.tsx
'use client';
...
export function GoalContributionDialog({ isOpen, onClose, goal, userAccounts, onSuccess }: GoalContributionDialogProps) {
  ...
  const onSubmit = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await api.post(`/familia/${goal.clanId}/goals/${goal.id}/contribute`, values);
      toast({ title: 'Contribuição realizada!', description: `Sua contribuição para a meta "${goal.name}" foi registrada.` });
      onSuccess();
      onClose();
      form.reset();
    } catch (error: any) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Contribuir para a Meta: {goal.name}</DialogTitle>
          <DialogDescription>Ajude sua família a alcançar este objetivo.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
