...
function GamificationModeForm({ user }: { user: User }) {
  ...
  const options = [
    { value: 'FULL', title: 'Modo Jornada', description: 'Experiência completa com missões, conquistas e eventos.' },
    { value: 'LITE', title: 'Modo Lite', description: 'Mantém XP e conquistas visíveis, mas reduz elementos épicos.' },
    { value: 'OFF', title: 'Modo Financeiro', description: 'Interface tradicional sem referências a RPG. O XP continua sendo armazenado em segundo plano.' },
  ] as const;
      ...
      <form onSubmit={form.handleSubmit(onSubmit)}>
        <Card>
          <CardHeader>
            ...
            <CardDescription>Escolha entre a experiência gamificada completa, uma versão leve ou o modo financeiro clássico.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <FormField
                ...
                <FormItem>
                  <FormControl>
                    <RadioGroup value={field.value} onValueChange={field.onChange} className="space-y-3">
                      {options.map((option) => (
                        <div key={option.value} className="flex items-start gap-3 rounded-xl border p-4">
                          <RadioGroupItem value={option.value} id={`mode-${option.value}`} className="mt-1" />
                          <label htmlFor={`mode-${option.value}`} className="space-y-1 leading-tight">
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid gap-8 lg:grid-cols-[260px,1fr]">
        <nav className="rounded-2xl border bg-card p-4 h-max sticky top-24">
