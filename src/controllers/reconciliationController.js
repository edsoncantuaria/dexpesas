...
class ReconciliationController {
    ...
    // Divide as transações em chunks e adiciona um job na fila para cada chunk
    async createAllFromImported(req, res, next) {
        const { reconciliationId } = req.params;
        const userId = req.user.id;
        const CHUNK_SIZE = 50; // Processa 50 transações por vez
        ...
        try {
            ...
            await prisma.reconciliation.update({ where: {id: reconciliationId}, data: { status: 'PROCESSING' }});

            res.status(202).json({ message: `${jobsAdded} lotes de transações foram adicionados à fila para processamento.` });
